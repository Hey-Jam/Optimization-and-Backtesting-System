...
public:
    ...
    Portfolio* optimize(vector<string>& tickers, boost::gregorian::date Start, boost::gregorian::date End) {
        ...
        vector<boost::gregorian::date> DateArray=stockpool->getTradingDates(Start,End);
        ...
        gsl_matrix_memcpy(closereturn, &next_price.matrix);
        ...
        double sum = 0;
        for(int i=0; i<num; i++) sum += gsl_vector_get(gsl_multimin_fminimizer_x(Optimize), i);
        gsl_vector_scale(gsl_multimin_fminimizer_x(Optimize), 1.0/sum);

        Portfolio *OptimizedPortfolio = new Portfolio(*(DateArray.end()-1), tickers, gsl_multimin_fminimizer_x(Optimize));
