...
public:
  ...
  // getter
  int getSize() const { return stockpool.getsize();}

  Account optimize(vector<string> tickers) const {
    /* the main function of ClassicMarkowitz class, implement the optimization with Markowitz method */

    gsl_matrix *closeprice = stockpool.getprice(tickers); // this part needed to be completed by Jam, column is the ticker, row is the date
    ...
    gsl_matrix_view next_price = gsl_matrix_submatrix(closeprice, 0, 0, date_length - 1, num);
    // suppose closeprice(0,0) is the latest price of the first ticker
    gsl_matrix_view last_price = gsl_matrix_submatrix(closeprice, 1, 0, date_length - 1, num);
    gsl_matrix_div_elements(&next_price.matrix, &last_price.matrix);

    gsl_matrix *closereturn = gsl_matrix_alloc(date_length - 1, num);
    gsl_matrix_mmcpy(&next_price.matrix, closereturn);
